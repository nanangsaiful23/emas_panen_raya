    ...
    public function export(Request $request)
    {
        ...
        foreach($transactions as $transaction)
        {
            foreach($transaction->detailsWithDeleted() as $detail)
            {
                ...
                array_push($result, [displayDate($transaction->created_at), $detail->good->code, $detail->good->name, $detail->good->weight, $detail->percentage->name, $detail->buy_price, $detail->selling_price, $status]);
            }
        }

        return Excel::download(new TransactionExport($result), 'Data Transaksi ' . $request->start_date . ' - ' . $request->end_date . '.xlsx');
    }
}
